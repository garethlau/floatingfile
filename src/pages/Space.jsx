...
import { saveBlob, getCodeFromWindow } from "../utils";
...
export default function Space() {
	...
	useEffect(() => {
		setCode(getCodeFromWindow(window));
	}, []);

	useDocumentTitle(`floatingfile | ${code}`);

	const { data: space, status: spaceStatus, refetch: refetchSpace } = useSpace(getCodeFromWindow(window));
	...
	const [removeFiles, { status: removeFilesStatus }] = useRemoveFiles({
		code: getCodeFromWindow(window),
		onSuccess: () => setSelected([]),
	});

	const [uploadFiles, { status: uploadStatus }] = useUploadFiles({
		code: getCodeFromWindow(window),
	});
